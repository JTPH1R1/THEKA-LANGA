-- Add SET row_security = off to every admin SECURITY DEFINER function.
-- Supabase's postgres role is not a true BYPASSRLS superuser, so SECURITY DEFINER
-- alone does not bypass RLS — the explicit SET is required.

CREATE OR REPLACE FUNCTION core.admin_get_stats()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = core, sacco, finance, kyc, system, public
...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = core, sacco, public
SET row_security = off
AS $$
BEGIN
  IF NOT core.is_admin() THEN RAISE EXCEPTION 'Unauthorized'; END IF;
  ...
  SELECT jsonb_build_object(
    ...
    'group_count',       (SELECT COUNT(*)::int FROM sacco.group_members gm WHERE gm.profile_id = p.id AND gm.status = 'active')
  )
  FROM core.profiles p
  WHERE (p_search IS NULL OR p.full_legal_name ILIKE '%' || p_search || '%' OR p.email ILIKE '%' || p_search || '%')
...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = core, kyc, public
SET row_security = off
AS $$
BEGIN
  IF NOT core.is_admin() THEN RAISE EXCEPTION 'Unauthorized'; END IF;
...
BEGIN
  IF NOT core.is_admin() THEN RAISE EXCEPTION 'Unauthorized'; END IF;
  UPDATE kyc.profiles
  SET verification_status = 'rejected', reviewed_at = now(), internal_notes = p_reason
  WHERE profile_id = p_profile_id;
END;
$$;
...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = core, audit, public
SET row_security = off
AS $$
BEGIN
  IF NOT core.is_admin() THEN RAISE EXCEPTION 'Unauthorized'; END IF;
...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = core, system, public
SET row_security = off
AS $$
BEGIN
  IF NOT core.is_admin() THEN RAISE EXCEPTION 'Unauthorized'; END IF;
