-- Phase 8: Contributions — RLS policies and RPCs
...
-- ─── RLS: finance.contributions ──────────────────────────────────────────────

-- Active group members can read all contributions for their group
CREATE POLICY "contributions: group member read"
...
-- Officers can insert and update contributions
...
-- ─── RLS: finance.contribution_fines ─────────────────────────────────────────

CREATE POLICY "contribution_fines: group member read"
  ON finance.contribution_fines FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM sacco.group_members gm
      WHERE gm.group_id = contribution_fines.group_id
        AND gm.profile_id = auth.uid()
        AND gm.status = 'active'
    )
  );

CREATE POLICY "contribution_fines: officer insert"
