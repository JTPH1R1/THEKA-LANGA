...
const RISK_COLOR: Record<string, string> = {
  low: 'text-emerald-400', medium: 'text-amber-400', high: 'text-rose-400', unknown: 'text-slate-500',
}
    ...
    <div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-100">KYC Review Queue</h1>
        ...
        {isLoading && (
          <div className="space-y-3">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="rounded-xl border border-slate-700 bg-slate-800/30 p-4 animate-pulse">
        ...
        <div className="space-y-3">
          {queue.map((entry) => (
            <div key={entry.profileId} className="rounded-xl border border-slate-700 bg-slate-800/30 p-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-slate-100 font-medium">{entry.fullLegalName}</p>
                  ...
                  {entry.submittedAt && (
                    <p className="text-xs text-slate-600 mt-1">Submitted {formatDate(entry.submittedAt)}</p>
                  )}
                </div>
                <div className="flex gap-2 shrink-0">
                  <Button
                    ...
                    variant="outline"
                    className="border-rose-800 text-rose-400 hover:bg-rose-900/20 gap-1.5"
                  >
